from neutral_coupling.gkeyll_coupling.geometry import gkeyll_geom as gkeyllGeom
import numpy as np
...
gg = gkeyllGeom.gkeyllGeom(
    str(Path(__file__).parents[1] / "geometry" / "stored_data" / "gkeyllGeometry.pkl")
)



# Loop over the entire B2 grid and get the psi,theta coordinates (and block number)
...
#
# np.save("gkeyll_b2_coordmapping.npy", ptb)
...
#        ptb[i,j] = gg.psitheta(R[i,j], Z[i,j])
#
# np.save("gkeyll_b2_coordmapping_radialfaces.npy", ptb)
...
#        print(i,j)
#        ptb[i,j] = gg.psitheta(R[i,j], Z[i,j])
# np.save("gkeyll_b2_coordmapping_parallelfaces.npy", ptb)
