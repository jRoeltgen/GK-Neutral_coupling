import numpy as np
from . import gkeyll_tracer as gkeyllTracer
import os
import pickle


_LEGACY_PICKLE_MODULES = {
    "gkeyllGeometry.gkeyllGeom": (
        "neutral_coupling.gkeyll_coupling.geometry.gkeyll_geom"
    ),
    "gkeyllGeometry.gkeyllEFIT": (
        "neutral_coupling.gkeyll_coupling.geometry.gkeyll_efit"
    ),
    "gkeyllGeometry.gkeyllTracer": (
        "neutral_coupling.gkeyll_coupling.geometry.gkeyll_tracer"
    ),
}


class _GeometryUnpickler(pickle.Unpickler):
    """Load geometry files written before the package reorganization."""

    def find_class(self, module, name):
        module = _LEGACY_PICKLE_MODULES.get(module, module)
